...
import 'package:am_innnn/data/checkbookmark_stream.dart';
...
class _AddBookMArkWidgetState extends State<AddBookMArkWidget> {
  ...
  int? userId;
  bool? isFav = false;
  CheckBookmarkStream checkBookmarkStream = CheckBookmarkStream();
  ...
  void initState() {
    ...
    // fetchData();
    if (_isLogin) {
      checkBookmarkStream.checkBookmark(appData.read(kKeyUserID).toString(),
          widget.newsId.toString(), _authToken);
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    // }
    return _isLogin
        ? StreamBuilder<CheckBookmarkModel>(
            stream: checkBookmarkStream.broadCastStream,
            builder: (context, AsyncSnapshot<CheckBookmarkModel> snapshot) {
              if (snapshot.hasData) {
                final data = snapshot.data!;
                bool isBookmarked = data.status ?? false;
                        ...
                        if (_isLogin) {
                          ...
                          UserData.addBookMark(
                            ...
                            if (value == 'Bookmark added successfully') {
                              ...
                              checkBookmarkStream.checkBookmark(
